...
public class PathFind {
	...
	public List<AirwayNode> dijkstra(AirwayNode start,AirwayNode target){
		...
		Map<AirwayNode,Double> costs = new HashMap<AirwayNode, Double>();
		
		Map<AirwayNode,AirwayNode> previous = new HashMap<AirwayNode, AirwayNode>();
		for(AirwayNode node:graph.getNodes()) {
		...
		while(!heap.isEmpty()) {
			HeapNode entry = heap.extractMin();
			if(entry==null) {
				break;
			}
			AirwayNode current = entry.getAirWayNode();
